...
\end{verbatim}
  ...
  \item \textbf{Relationship 2: player size and rebounding (scatterplot baseline).}

  Make a scatterplot of \texttt{PLAYER\_WEIGHT} (x) vs \texttt{REB} (y).
...
\end{verbatim}
  ...
  \noindent \textbf{Write-up (3--5 sentences):}
  \begin{itemize}
    \item What direction and strength does the relationship show?
    \item Give one plausible basketball reason heavier players might (or might not) average more rebounds.
  \end{itemize}

  \noindent Save as:
  \[
    \texttt{figures/size\_reb\_scatter.png}
  \]

  \item \textbf{Relationship 3: age and scoring efficiency (polynomial fit).}

  Make a scatterplot of \texttt{AGE} (x) vs \texttt{TS\_PCT} (y) with a \textbf{quadratic} (degree-2 polynomial) smoother.
...
\end{verbatim}
  ...
  \begin{itemize}
    \item Use \texttt{geom\_point()} with an overplotting fix (e.g., \texttt{alpha} and/or smaller \texttt{size}).
    \item Format \texttt{TS\_PCT} as a percent if you can.
    \item Add a quadratic smoother with standard errors: \texttt{geom\_smooth(method = "lm", formula = y \textasciitilde{} poly(x, 2), se = TRUE)}.
    \item Use \texttt{theme\_classic()}.
  \end{itemize}

  \noindent \textbf{Write-up (3--5 sentences):}
  \begin{itemize}
    \item Does a quadratic fit capture the shape better than a straight line would?
    \item Where is the SE ribbon widest, and what does that tell you about sample density at those ages?
    \item Based on the curve, at roughly what age does scoring efficiency appear to peak?
  \end{itemize}

  \noindent Save as:
  \[
    \texttt{figures/age\_ts\_poly2\_se.png}
  \]
