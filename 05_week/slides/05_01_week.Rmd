...
output:
  xaringan::moon_reader:
    css: [default, metropolis, metropolis-fonts, plsc498.css, slides.css]
    nature:
...
df_time <- data.frame(
  t = 1:200,
  y =  10 + 0.03 * 1:200 + 1.5 * sin(2 * pi * 1:200 / 12)  + arima.sim(model = list(ar = 0.7), n = 200, sd = 0.8))
df_unc <- data.frame(
  label = c("A","B","C","D","E"),
...
# Histogram bin size formulas

Common ways to choose bin width $h$ (or number of bins ($k$)):


- **Sturges (via number of bins):**  
  
  $$
  k = \lceil \log_2(n) + 1 \rceil, \qquad h = \frac{\max(x)-\min(x)}{k}
  $$
...
- **Freedman–Diaconis (bin width; robust):**  
  
  
  $$
  h = 2 \, \frac{\mathrm{IQR}(x)}{n^{1/3}}
  $$ 
  where $\mathrm{IQR}(x) = Q3 - Q1$.


**Practical note:** Freedman–Diaconis is often a good default because it is less sensitive to outliers than Scott.
...
.center[
<img src="height_men.png" style="width:40%;">
]
...
- **IQR (Q3 − Q1):** height of the box (the middle 50% of the data)  
- **Whiskers:** lines that extend from the box to show the typical range of the data  
  - **Lower whisker:** goes down to the *lowest data point that isn’t unusually low*  
  - **Upper whisker:** goes up to the *highest data point that isn’t unusually high*  
  - “Unusually” is defined by the common rule: anything more than **1.5 × IQR** below Q1 or above Q3 is treated as a potential outlier
...
.center[
<img src="nuclear_weapons.png" style="width:40%;">
]
...
# Confidence intervals (what they are)


A **confidence interval (CI)** describes the range of values that are  
**consistent with the data and the statistical model**. For a 95% confidence interval, if repeated many times, 95% of the intervals constructed this way contain the true value

What a CI **is**:
- A measure of **estimation uncertainty**
- A way to compare estimates across groups or time
- Wider intervals = more uncertainty  
- Narrower intervals = more precision
- ❌ The probability that the true value is in this specific interval  or a statement about individual observations

**Visual interpretation:**
- The point = estimated value , the bars = uncertainty around that estimate, overlapping intervals suggest limited evidence of differences (not a formal test)

**Key idea:**  
Confidence intervals show how much trust you should place in the estimate,  
not just where the estimate lies.
...
.center[
<img src="earth_minerals.png" style="width:35%;">
]
