...
\title{Take-Home Midterm: Visualizing War, Regime Type, Distance, and Trade (R + GitHub / Canvas)}
\author{}
\date{}
...
\begin{itemize}
\item You \textbf{may not} use AI tools (e.g., ChatGPT, Copilot, or similar systems) to write your answers. Your written interpretations and explanations must be your own.
\item The exam is open book, note, and web. \textbf{However, you are responsible for understanding and explaining your code and results.}
...
\medskip
\noindent \textbf{Submission options (choose ONE): Canvas \emph{or} GitHub.}

\begin{itemize}
  \item \textbf{Option A (Canvas submission).}
  \begin{itemize}
    \item Upload \textbf{one ZIP file} to Canvas.
    \item The ZIP must contain (at minimum) these folders and files:
    \begin{itemize}
      \item \texttt{scripts/midterm.R} \textit{(preferred)} or \texttt{midterm.qmd} / \texttt{midterm.Rmd}
      \item \texttt{outputs/writeup.md}
      \item \texttt{figures/} (your saved figures)
    \end{itemize}
    \item \textbf{Do not} upload many loose files individually. Submit a single ZIP that preserves the folder structure.
  \end{itemize}

  \item \textbf{Option B (GitHub submission).}
  \begin{itemize}
    \item Push your work to your GitHub repo with the same required structure:
    \begin{itemize}
      \item \texttt{scripts/midterm.R} \textit{(preferred)} or \texttt{midterm.qmd} / \texttt{midterm.Rmd}
      \item \texttt{outputs/writeup.md}
      \item \texttt{figures/} (your saved figures)
...
\begin{itemize}
  ...
  \item Organize your work with \texttt{scripts/}, \texttt{outputs/}, and \texttt{figures/}.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Submission proof (required). Choose the proof that matches your submission method.}

  \begin{itemize}
    \item \textbf{If you submit via GitHub:} In your write-up, paste:
    \begin{itemize}
    ...
    \item \textbf{If you submit via Canvas:} In your write-up, include:
    \begin{itemize}
      \item a short \textbf{file tree} of what is inside your ZIP (you can type it manually), showing \texttt{scripts/}, \texttt{outputs/}, and \texttt{figures/}, and
      \item the \textbf{exact filename} of the ZIP you uploaded to Canvas (e.g., \texttt{Lastname\_Firstname\_Midterm.zip}).
    \end{itemize}
  \end{itemize}
...
\begin{itemize}
  ...
  \item You submitted via \textbf{either} Canvas \textbf{or} GitHub (not both)
  \item You included the correct submission proof for your chosen method (GitHub proof \emph{or} Canvas ZIP proof)
\end{itemize}
