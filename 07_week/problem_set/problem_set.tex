...
\noindent \textbf{Code snippet expectation.} For Questions 2--6, include \textbf{small R snippets (2--4 lines)} near the relevant instructions as reminders of correct syntax. These are not full solutions---just scaffolding.
...
\medskip
\noindent \textbf{R snippet (for proofs in your write-up):}
\begin{verbatim}
getwd()
list.files()
list.files("data")
\end{verbatim}
  ...
  \begin{enumerate}
    ...
    \item Verify the key columns exist: \texttt{country}, \texttt{year}, \texttt{battle\_deaths}, \texttt{region}, \texttt{income}.
...
\medskip
\noindent \textbf{R snippet (load + quick checks):}
\begin{verbatim}
...
\medskip
\noindent \textbf{R snippet (proof statistics):}
\begin{verbatim}
dim(df)
...
\medskip
\noindent \textbf{R snippet (histogram + save):}
\begin{verbatim}
p_hist <- ggplot(df, aes(x = battle_deaths)) +
...
\medskip
\noindent \textbf{R snippet (density + same x limits idea):}
\begin{verbatim}
p_den <- ggplot(df, aes(x = battle_deaths)) + 
geom_density() + coord_cartesian(xlim = c(0, 5000)) + 
theme_classic()
...
\medskip
\noindent \textbf{R snippet (correct ggplot Q--Q syntax):}
\begin{verbatim}
p_qq <- ggplot(df, aes(sample = battle_deaths)) + 
stat_qq() + 
stat_qq_line() + 
theme_classic()
...
\medskip
\noindent \textbf{R snippet (facet by income):}
\begin{verbatim}
p_inc <- ggplot(df, aes(x = battle_deaths)) + 
...
\medskip
\noindent \textbf{R snippet (facet by region):}
\begin{verbatim}
p_reg <- ggplot(df, aes(x = battle_deaths)) + 
geom_histogram(bins = 30) + 
facet_wrap(~ region) + 
theme_classic()
...
\end{verbatim}
    ...
    \begin{itemize}
      \item if using GitHub: the output of \texttt{git status} after committing (clean working tree) and \texttt{git log -1},
...
\medskip
\noindent \textbf{R snippet (ridgeline starter):}
\begin{verbatim}
library(ggridges)
